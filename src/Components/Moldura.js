import React from "react";

function Moldura() {
  return (
    <div class="row">
      {/* 1ª Coluna */}
      <div class="column">
        <img
          class="moldura"
          src="https://images.unsplash.com/photo-1518098268026-4e89f1a2cd8e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&w=1000&q=80"
          alt="foto 1"
        />
        <img
          class="moldura"
          src="https://www.aboutnewzealand.com/wp-content/uploads/2017/06/Hackett-Ledge-Bungy.jpg"
          alt="foto 2"
        />
        <img
          class="moldura"
          src="http://info.annimaddaloni.de/wp-content/uploads/2018/10/Lake_Mountains_Winter_438866.jpg"
          alt="foto 3"
        />
        <img
          class="moldura"
          src="https://thebig.co/images/blogs/vertical_panorama02.jpg"
          alt="foto 4"
        />
      </div>

      {/* 2ª Coluna */}
      <div class="column">
        <img
          class="moldura"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS_UVkBN5Wr0cZI6NtQZTWDIfV-DL8WWUkSs1YnLPJY3NSVixBw"
          alt="foto 1"
        />
        <img
          class="moldura"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRnqfq_kvfecfDCdByGeVyG4Vk4th56dk-Yoqb9V2UKzrEGg1fKwg"
          alt="foto 2"
        />
        <img
          class="moldura"
          src="https://images.unsplash.com/photo-1469827160215-9d29e96e72f4?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&w=1000&q=80"
          alt="foto 3"
        />
        <img
          class="moldura"
          src="https://cdn-images-1.medium.com/max/2400/1*Yd2JS7Jo7W6DHf6VBmGnIg.jpeg"
          alt="foto 4"
        />
        <img
          class="moldura"
          src="https://media-cdn.tripadvisor.com/media/photo-s/0c/16/e9/1f/sunset-on-lake-mi-beach.jpg"
          alt="foto 5"
        />
      </div>

      {/* 3ª Coluna */}
      <div class="column">
        <img
          class="moldura"
          src="https://www.cpre.org.uk/media/k2/items/cache/72700a14a2a02b35fe25ed088f8895bc_M.jpg"
          alt="foto 1"
        />
        <img
          class="moldura"
          src="https://www.geniusjourney.com/pictures/i6/fn/i6fnoc4e7d3290b07sb3/New_Zealand-Tauranga_3.jpg"
          alt="foto 2"
        />
        <img
          class="moldura"
          src="https://upload.wikimedia.org/wikipedia/commons/9/91/F-15_vertical_deploy.jpg"
          alt="foto 3"
        />
      </div>

      {/* 4ª Coluna */}
      <div class="column">
        <img
          class="moldura"
          src="https://www.w3schools.com/w3images/rocks.jpg"
          alt="foto 1"
        />
        <img
          class="moldura"
          src="https://learn.zoner.com/wp-content/uploads/2018/08/landscape-photography-at-every-hour-part-ii-photographing-landscapes-in-rain-or-shine-683x390.jpg"
          alt="foto 2"
        />
        <img
          class="moldura"
          src="data:image/jpeg;base64,"
          alt="foto 4"
        />
      </div>
    </div>
  );
}

export default Moldura;
